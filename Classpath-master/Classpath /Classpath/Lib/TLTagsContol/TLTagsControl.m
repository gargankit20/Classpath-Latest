...
- (void)commonInit {
    ...
    tagInputField_.font = [UIFont fontWithName:@"SFProText-Regular" size:10];
...
- (void)reloadTagSubviews {
    ...
    UIColor *tagBackgrounColor = _tagsBackgroundColor != nil ? _tagsBackgroundColor : [UIColor colorWithRed:0.32 green:0.71 blue:0.72 alpha:1.0];
...
- (void)deleteTagButton:(UIButton *)sender {
    ...
    NSInteger index = [tagSubviews_ indexOfObject:view];
    NSString *str = [_tags objectAtIndex:index];
    [_tags removeObjectAtIndex:index];
    [tapDelegate deleteTagsControl:self tappedAtIndex:index deleteTagText:str];
    [self reloadTagSubviews];
}
